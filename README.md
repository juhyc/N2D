...
1. **문제 제기**
    ...
    그리고 쏘카에서 이 이미지들을 딥러닝을 이용해 파손 부위를 탐지하는 모델 개발했습니다.

     [*Semantic Segmentation을 활용한 차량 파손 탐지 딥러닝 모델 개발기*](https://tech.socarcorp.kr/data/2020/02/13/car-damage-segmentation-model.html) (👈 해당 쏘카 블로그 포스팅으로 가기)
    
    해당 프로젝트 과정에서 육안으로 파손 여부를 확실히 판별할수 있는 이미지가 2000장.
